...
class Turbosmtp_Validated_Emails_Table extends WP_List_Table {
	...
	protected function get_views() {
		...
		foreach ($statuses as $status) {
			...
			$status_links[$status['status']] = sprintf('<a style="'.($is_status_selected ? 'font-weight:bold': '').'" href="%s">%s (%d)</a>', admin_url( 'options-general.php?page=email-validation-settings&subpage=history&status='.$status['status'] ), ucfirst($status['status']), $status['total']);
		}
